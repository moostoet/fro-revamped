...
static void CB2_ReturnToPartyMenuWhileLearningMove(void)
{
    ...
    if (learnMethod == LEARN_VIA_TMHM && moveIdx != MAX_MON_MOVES)
    {
        ...
        if (ItemId_GetFieldFunc(gSpecialVar_ItemId) == FieldUseFunc_RareCandy && gPartyMenu.menuType == PARTY_MENU_TYPE_FIELD && CheckBagHasItem(gSpecialVar_ItemId, 1))
...
static void Task_HandleStopLearningMoveYesNoInput(u8 taskId)
{
    ...
    switch (Menu_ProcessInputNoWrapClearOnChoose())
    {
    case 0: // Yes – “Don’t learn the move”
        GetMonNickname(mon, gStringVar1);
        StringCopy(gStringVar2, gMoveNames[gPartyMenu.learnMoveId]);
...
void ItemUseCB_RareCandy(u8 taskId, TaskFunc func)
{
    ...
    u16 item = gSpecialVar_ItemId;
    bool8 noEffect;
    u16 curLevel = GetMonData(mon, MON_DATA_LEVEL);
    ...
    else if (curLevel + 1 > GetCurrentLevelCap())
    {
        noEffect = TRUE;
    }
#endif
    ...
    {
        // Instead of scheduling an item animation task and then running RareCandyStep,
        // we immediately run the Rare Candy effect code inline.
        struct PartyMenuInternal *ptr = sPartyMenuInternal;
        ...
        // Continue with the level-up stats page immediately. (Previously, the battery of item animation tasks
        // might have eventually switched to this task.) You can now simply set the task callback:
...
static void PartyMenuTryEvolution(u8 taskId)
{
    ...
    {
        FreePartyPointers();
        if (ItemId_GetFieldFunc(gSpecialVar_ItemId) == FieldUseFunc_RareCandy &&
...
static bool8 MonCanEvolve(void)
{
    if (!IsNationalPokedexEnabled() && GetEvolutionTargetSpecies(&gPlayerParty[gPartyMenu.slotId], EVO_MODE_ITEM_USE, gSpecialVar_ItemId) > KANTO_DEX_COUNT)
        return FALSE;
    else
        return TRUE;
}
